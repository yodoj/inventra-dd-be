...
public interface AsetRuanganRepository extends JpaRepository<AsetRuangan, UUID> {
        @Query(value = "SELECT ar.*, a.* FROM aset_ruangan ar JOIN aset a ON ar.id_aset = a.id_aset WHERE " +
                        ...
                        "(:search IS NULL OR :search = '' OR a.nama_aset ILIKE CONCAT('%', :search, '%') OR a.kode_aset ILIKE CONCAT('%', :search, '%'))", countQuery = "SELECT count(*) FROM aset_ruangan ar JOIN aset a ON ar.id_aset = a.id_aset WHERE "
                                        +
                                        "(:unit IS NULL OR :unit = '' OR a.unit ILIKE :unit) AND " +
                                        "(:kategori IS NULL OR :kategori = '' OR a.kategori_aset = :kategori) AND " +
                                        "(:status IS NULL OR :status = '' OR a.status_aset = :status) AND " +
                                        "(:search IS NULL OR :search = '' OR a.nama_aset ILIKE CONCAT('%', :search, '%') OR a.kode_aset ILIKE CONCAT('%', :search, '%'))", nativeQuery = true)
        ...
        @Query("SELECT ar FROM AsetRuangan ar WHERE " +
               "(:unit IS NULL OR :unit = '' OR ar.unit = :unit) AND " +
               "ar.statusAset = 'TERSEDIA'")
